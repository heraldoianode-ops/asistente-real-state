...
interface Agent { id: string; email: string; full_name: string | null; role: string; is_active: boolean; wa_contact_id: string | null; created_at: string }
...
export default function AgentsPage() {
  ...
  async function loadAgents() {
    setLoading(true)
    const { data } = await createClient().from('users').select('id,email,full_name,role,is_active,wa_contact_id,created_at').neq('role','admin').order('created_at',{ascending:false})
    setAgents(data ?? [])
    setLoading(false)
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/create-agent`, {
        method: 'POST',
        ...
        <div className="card-creatio overflow-hidden">
          ...
          : <table className="w-full text-sm">
              <thead><tr className="border-b border-[hsl(var(--border))] bg-[hsl(var(--secondary))]">
                {['Nombre','Email','WA Contact','Estado','Acción'].map(h=><th key={h} className="px-4 py-3 text-left text-xs font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider">{h}</th>)}
              </tr></thead>
