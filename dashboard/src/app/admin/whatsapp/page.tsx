...
export default function WhatsAppPage() {
  ...
  async function loadNumbers() {
    ...
    const { data } = await supabase
      ...
      .select('id, agent_id, phone_number, phone_number_id, display_name, is_active')
      .order('phone_number')
    setNumbers(data ?? [])
    setLoading(false)
  }
  ...
  async function handleToggle(n: WANumber) {
    const supabase = createClient()
    await supabase.from('whatsapp_numbers').update({ is_active: !n.is_active }).eq('id', n.id)
    await loadNumbers()
  }
    ...
    <AdminGuard>
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold text-[hsl(var(--foreground))]">Números WhatsApp</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[hsl(var(--border))] bg-[hsl(var(--secondary))]">
                  {['Número', 'Phone ID', 'Nombre', 'Agent ID', 'Estado', 'Acción'].map((h) => (
              ...
              <tbody className="divide-y divide-[hsl(var(--border))]">
                {numbers.map((n) => (
                  <tr key={n.id} className="hover:bg-[hsl(var(--secondary))] transition-colors">
                    ...
                    <td className="px-4 py-3">{n.display_name ?? <span className="text-[hsl(var(--muted-foreground))]">—</span>}</td>
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            ...
            >
              <div className="px-6 py-4 border-b border-[hsl(var(--border))]">
                <h2 className="text-base font-semibold">Registrar número WhatsApp</h2>
              </div>
                ...
                {(['agent_id', 'phone_number', 'phone_number_id', 'display_name'] as const).map((field) => (
                  <div key={field}>
                    <label className="block text-sm font-medium mb-1.5 capitalize">
                      {field.replace(/_/g, ' ')}
                      {field !== 'display_name' && <span className="text-red-500 ml-0.5">*</span>}
                    </label>
                    <input
                      type="text"
                      value={form[field]}
