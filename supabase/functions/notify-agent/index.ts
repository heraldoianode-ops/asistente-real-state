...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: waNumber } = await supabase
      ...
      .select('phone_number')
      .eq('agent_id', listing_agent_id)
      .eq('is_active', true)
      .maybeSingle()

    if (!waNumber) {
      return new Response(
        JSON.stringify({ ok: false, reason: 'No active WhatsApp number for agent' }),
    ...
    if (!gatewayUrl) {
      return new Response(
        JSON.stringify({ ok: false, reason: 'WHATSAPP_GATEWAY_URL not set' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    const message = `🏠 *Nueva coincidencia detectada*\n\n${explanation}\n\n_Ingresá al dashboard para ver los detalles._`

    const res = await fetch(`${gatewayUrl}/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ to: waNumber.phone_number, message }),
    })

    return new Response(
      JSON.stringify({ ok: res.ok, match_id }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  } catch (err) {
    return new Response(
