...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const match of (matches ?? [])) {
      ...
      const { data: inserted } = await supabase.from('cross_agent_matches').insert({ property_id, buyer_client_id: match.client_id, listing_agent_id: prop.listing_agent_id, buyer_agent_id: match.agent_id, match_score: match.similarity, match_explanation: explanation, buyer_client_name: match.client_name, status: 'pending' }).select('id').single()
      if (inserted) {
        ...
        if (gw) fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/notify-agent`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')}` }, body: JSON.stringify({ match_id: inserted.id, listing_agent_id: prop.listing_agent_id, explanation }) }).catch(()=>null)
      }
    }
